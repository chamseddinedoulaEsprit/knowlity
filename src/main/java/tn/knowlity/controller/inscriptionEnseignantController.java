...
public class inscriptionEnseignantController {
    private static final Logger LOGGER = Logger.getLogger(inscriptionEnseignantController.class.getName());
    ...
    @FXML private TextField email, nom, prenom, numTelephone, dateNaissance, localisation, password1;
    @FXML private PasswordField password, confirmPassword;
    @FXML private Label imagePathLabel, errornom, imageError, prenomError, numTelephoneError, dateNaissanceError, localisationError, passwordError, confirmPasswordError, emailError, specialiteError;
        ...
        try {
            ...
            new Thread(() -> {
                try {
                    ...
                    newUser.setRoles(new String[]{"Enseignant"});
    ...
    public void afficher() throws ParseException, SQLException, IOException {
        ...
        if (specialite == null || specialite.isEmpty() || specialite.length() < 3) {
            compteur++;
            showError("Spécialité invalide (minimum 3 caractères)", specialiteError);
            LOGGER.warning("Invalid specialite: " + specialite);
        } else {
        ...
        if (compteur == 0) {
            ...
            String[] roles = isGoogleSignIn ? currentUser.getRoles() : new String[]{"Enseignant"};
            User user = new User(
            ...
            LOGGER.info("Teacher registered successfully: " + emailInput);
            pagelogin();
        } else {
